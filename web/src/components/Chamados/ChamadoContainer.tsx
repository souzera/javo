...
export default function ChamadoContainer({ chamado }: ChamadoContainerProps) {
  let stylePrioridade =
    "px-4 rounded-2xl text-white font-bold stroke-[13vw] stroke-white";
  let styleContainer =
    "flex flex-col w-auto h-auto cursor-move text-white text-ellipsis p-4 gap-2 rounded-2xl transition ease-in-out duration-1000 hover:scale-105 overflow-hidden border-2 bg-zinc-900";
  ...
  const [userOrigin, setUserOrigin] = useState<Usuario>({
    id_profile: "",
    user: "undefined",
    url_avatar: "",
  });
      ...
      <Draggable>
        <div className={styleContainer}>
          ...
          <div className="h-auto">
            ...
            {chamado.desc && <h3 className="text-clip">{chamado.desc}</h3>}
          </div>
          <div className="flex justify-between gap-2">
            <span
              className="font-thin"
              onClick={() => {
                console.log("abrir perfil do usuario");
              }}
            >
              {userOrigin?.user}
            </span>
